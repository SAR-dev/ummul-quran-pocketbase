                ...
                {invoices.map((invoice, i) => (
                    <div className="grid grid-cols-2 xl:grid-cols-5 gap-2 px-5 py-3" key={i}>
                        ...
                        <div className='gap-3 flex xl:justify-end'>
                            <Link to={`/teacher/invoices/${invoice.id}`} className="btn btn-sm">Details</Link>
                            <a target='_blank' href={`${import.meta.env.VITE_API_URL}/teacher-receipt/${invoice.id}`} className="btn btn-info btn-sm">Receipt</a>
                        </div>
                    </div>
                ))}
            </div>
