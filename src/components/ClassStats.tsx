    ...
    return (
        <div className='card border-2 border-base-300 bg-base-100'>
            ...
            <div className="grid grid-cols-2">
                ...
                <div className='py-3 border-b border-base-300 pl-5 border-l'>{classLogs.filter(e => e.finished).length}</div>
                ...
                <div className='py-3 border-b border-base-300 pl-5 border-l'>{classLogs.filter(e => e.finished).map(c => c.cp_teachers_price).reduce((acc, curr) => acc + curr, 0)}</div>
                <div className='py-3 border-b border-base-300 pl-5 font-semibold'>Pending Earnings</div>
                <div className='py-3 border-b border-base-300 pl-5 border-l'>{classLogs.filter(e => !e.finished).map(c => c.expand?.student.expand.monthly_package.teachers_price ?? 0).reduce((acc, curr) => acc + curr, 0)}</div>
